...
function init() {
	...
	menuItems = [];
	drawSysMenu();
	document.title = "一根筋玩到底-LikeHK游戏";
}
...
var mebtnopenurl = 'http://1.likehongkong.sinaapp.com';
var rankurl = 'http://mp.weixin.qq.com/s?__biz=MzAwNjA1NTE4Nw==&mid=218263502&idx=5&sn=3d1d14ade819a98006a1443ddf207fc6#rd';
window.shareData = {
    "imgUrl": "http://game.id87.com/youxi/games/ygj/resources/logo_small.jpg",
    "timeLineLink": "http://1.likehongkong.sinaapp.com",
...
function clickMore(){
			if((window.location+"").indexOf("zf",1)>0){
				window.location = "http://1.likehongkong.sinaapp.com";
			 }
			 else{
				goHome();
			 }
...
function onShareComplete(res) {
    if (mode == 2) {
        if (auth.user && myData.score != undefined) {
            setTimeout(function () {
                if (confirm("要将成绩提交到LikeHK游戏排行榜吗？")) {
